...
- **Auth** — Supabase Auth login, session-protected dashboard routes (`middleware.ts`), role stored on a `profiles` table
- **Dashboard** — live stat cards, books-overview chart (week/month/year), recent activity, recently issued books, top books by category, and a recommendations widget — all reading real data, no mock/sample fallback
